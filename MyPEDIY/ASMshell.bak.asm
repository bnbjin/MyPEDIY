...
PUBLIC	C	ShellStart0
PUBLIC	C	ShellEnd0
PUBLIC	C	ShellStart
PUBLIC	C	ShellEnd
PUBLIC	C	ImportTableBegin
PUBLIC	C	ImportTableEnd
PUBLIC	C	SHELL_DATA_0
PUBLIC	C	SHELL_DATA_1
PUBLIC	C	TlsTable
...
next0:
	...
	; VirtualAlloc(0, ShellPackSize, MEM_COMMIT, PAGE_READWRITE)
	push	PAGE_READWRITE
	push	MEM_COMMIT
	...
	; 将外壳第二段地址放到ShellBase2，dll退出时会用到
	push	eax
	mov	dword ptr [ebp+(ShellBase2-ShellStart0)], eax

	; _aP_depack_asm(ShellBase + ebp, 前面分配的堆空间);
...
ShellStart LABEL	DWORD
	...
	; 复制ap_depack_asm和VirtualAlloc地址到第二段外壳的数据表中
	...
	; FileHandle = imagebase
	mov 	eax, [ebp+(imagebase-ShellStart0)]	
	mov	ebp, edx
	mov	dword ptr [ebp+(FileHandle-ShellStart)], eax

	; 如果是EXE文件，则用GetModuleHandle(NULL)获取模块基址
	...
	; *  解压缩各区块  *
	;for S_PackSection to 1
	;{
	;	VirtualAlloc(0, S_PackSection, MEM_COMMIT, PAGE_READWRITE)
	;	...
	;}
	...
	push	0
	call	dword ptr [ebp+(S_VirtualallocADDR-ShellStart)]
	pop	ebx
	mov	esi,eax
	mov	eax,ebx
	add	eax,ebp
...
fuapfdw_finished:
	inc     dword ptr [ebp+(ShellStep_2-ShellStart)]
	
	; ReturnOEP = OEP VA 
	mov	eax, dword ptr [ebp+(OEP-ShellStart)]
	add	eax, dword ptr [ebp+(FileHandle-ShellStart)]
	add	dword ptr [ebp+(ReturnOEP-ShellStart)],eax
	popad
ReturnOEP:
	ret
